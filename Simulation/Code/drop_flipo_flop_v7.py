...
# 4. Flick Parameters (v_x, v_z, omega are independent)
print("\n--- Stage 4: Flick Parameters ---")
print("Manual flick mode: v_x, v_z, and omega can be set independently.")
try:
	vx_input = input("Enter translational speed v_x in m/s [Default: 1.0]: ")
	v_x = float(vx_input) if vx_input.strip() else 1.0

	vz_input = input("Enter translational speed v_z in m/s [Default: 0.0]: ")
	v_z = float(vz_input) if vz_input.strip() else 0.0

	wx_input = input("Enter angular speed omega in rad/s [Default: 20.0]: ")
	omega = float(wx_input) if wx_input.strip() else 20.0
except ValueError:
	print("Invalid input. Using defaults: v_x=1.0, v_z=0.0, omega=20.0")
	v_x = 1.0
	v_z = 0.0
	omega = 20.0
