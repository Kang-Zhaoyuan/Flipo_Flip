...
def get_playback_speed(default_speed=1.0):
  speed_input = input("Enter playback speed multiplier [Default: 1.0]: ").strip()
  if not speed_input:
  ...
  try:
    ...
    if speed <= 0:
      print(f"Invalid speed '{speed_input}'. Using default {default_speed}x.")
      return default_speed
    return speed
  except ValueError:
    print(f"Invalid speed '{speed_input}'. Using default {default_speed}x.")
...
# 4. Flick Parameters (Corrected Axes)
print("\n--- Stage 4: Flick Parameters ---")
print("We will let the toy settle, then flick it perpendicular to its flat face.")
try:
    vy_input = input("Enter forward velocity (v_y) in m/s [Default: 1.5]: ")
    v_y = float(vy_input) if vy_input.strip() else 1.53
    
    # We ask for a positive value here for user intuition, 
    # and invert it mathematically in the physics loop
    ...
    print("Invalid input. Using defaults: v_y=1.5, omega_x=20.0")
    v_y, omega_x = 1.5, 20.0
...
while viewer.is_alive:
    ...
    while data.time < time_elapsed:
        ...
        if data.time >= settle_time and not flick_applied:
            # data.qvel is mapped to [vx, vy, vz, wx, wy, wz]
            # Translate along Y, rotate around X (negative for forward pitch)
            data.qvel[1] = v_y
            data.qvel[3] = -omega_x
