...
def derive_source_slug_from_raw_csv(csv_path: Path) -> str:
    stem = csv_path.stem
    if stem.endswith(RAW_CSV_SUFFIX):
        return stem
...
def discover_raw_csv_files(raw_dir: Path) -> List[Path]:
    csv_paths = sorted(path.resolve() for path in raw_dir.glob("*_raw_data.csv") if path.is_file())
    if not csv_paths:
        raise FileNotFoundError(
            f"No *_raw_data.csv files found under raw directory: {raw_dir}"
        )
    return csv_paths
...
def process_single_csv_file(
    ...
    source_slug = derive_source_slug_from_raw_csv(csv_path)
    ...
    for index, csv_path in enumerate(csv_paths, start=1):
        ...
        try:
            ...
            try:
                ...
                source_slug = csv_path.stem or "source"
