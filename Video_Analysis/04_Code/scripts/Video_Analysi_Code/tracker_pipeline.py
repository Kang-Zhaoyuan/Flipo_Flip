...
def _build_local_search_roi(
    ...
    radius = max(0, int(params.local_search_radius_px))
    pad = max(0, int(params.local_search_padding_px))

    x0 = max(0, x - radius - pad)
    y0 = max(0, y - radius - pad)
    x1 = min(frame_w, x + w + radius + pad)
    y1 = min(frame_h, y + h + radius + pad)
...
def run_tracker(
    ...
    while True:
        ...
        local_search_roi = _build_local_search_roi(
            state.last_object_bbox,
            frame.shape[:2],
            params,
        )
        ...
        if pink.found and pink.contour is not None:
            ...
            state.last_object_bbox = pink_bbox
            state.object_lost_streak = 0
        else:
            state.object_lost_streak += 1
            if state.object_lost_streak > params.local_search_max_lost_frames:
                state.last_object_bbox = None
